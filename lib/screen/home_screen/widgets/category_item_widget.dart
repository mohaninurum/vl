...
class CategoryItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      ...
      child: Stack(
        children: [
          //title.isNotEmpty ? Image.asset(image, height: media.height * 0.11) :
          Container(
            ...
            child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [image == "favorite" ? Icon(Icons.favorite, size: 50, color: Colors.white) : CachedNetworkImage(fit: BoxFit.cover, height: media.height * 0.10, imageUrl: image, placeholder: (context, url) => Center(child: CircularProgressIndicator()), errorWidget: (context, url, error) => Icon(Icons.error, size: 40, color: Colors.white)), SizedBox(height: media.height * 0.01), Text(title, style: TextStyle(color: AppTextColors.appTextColorWhite, fontWeight: FontWeight.w600)), language.isNotEmpty ? Text("($language)", style: TextStyle(color: AppTextColors.appTextColorWhite, fontWeight: FontWeight.w600, fontSize: 10)) : SizedBox.shrink()]),
          ),
