...
class TestContentDetialScreen extends StatefulWidget {
  TestContentDetialScreen({required this.language, required this.selectClassName, required this.selectChapterName, super.key, required this.tabtype});
...
class _VideoContentDetailScreenState extends State<TestContentDetialScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBarWidget(appTitle: '${widget.selectClassName.substring(0, indexget + 1)} ${widget.tabtype}'), backgroundColor: const Color(0xFFF2F5FA), body: SafeArea(child: Container(decoration: BoxDecoration(border: Border.all(color: AppColors.pramarycolor)), child: Padding(padding: const EdgeInsets.all(8.0), child: SfPdfViewer.network(pdfUrl)))));
  }
}
