...
import 'package:visual_learning/screen/quiz_screen/quize_list_screen.dart';
...
import '../test_paper/blocs/test_paper_bloc.dart';
import '../test_paper/blocs/test_paper_event.dart';
import '../test_paper/blocs/test_paper_state.dart';
import '../widgets/appBarWidget.dart';

class QuizMainScreen extends StatefulWidget {
  const QuizMainScreen({super.key, this.selectsName, this.id});
  final selectsName;
  final id;
  @override
  State<QuizMainScreen> createState() => _TestPaperScreenState();
}

class _TestPaperScreenState extends State<QuizMainScreen> {
              ...
              children: [
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.start, children: [Text(widget.selectsName, style: TextStyle(color: AppTextColors.appTextColorWhite, fontSize: 15, fontWeight: FontWeight.w600)), Text("For all classes", style: TextStyle(color: AppTextColors.appTextColorWhite, fontSize: 13, fontWeight: FontWeight.w600))]),
                      Spacer(),
                      Image.asset('assets/appicons/quiz_icon.png', height: height * 0.12),
                    ],
                  ),
                ),
                            ...
                            ? ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return Padding(
                                  ...
                                  child: InkWell(
                                    onTap: () {
                                      Navigator.push(context, MaterialPageRoute(builder: (context) => QuizeListScreen(selectClassesName: selectClass, id: state.filteredNotes[index].id, selectChapterName: state.filteredNotes[index].title, subject: selectSubject)));
                                    },
                                    child: Container(
                                      color: AppColors.lightpurplecolor,
                                      child: ListTile(
                                        dense: true,
                                        minTileHeight: height * 0.02,
                                        leading: const Icon(Icons.note_alt_outlined, color: Colors.purple),
                                        title: Text('${index + 1}. ${note.title}', style: TextStyle(fontSize: 13)),
                                        subtitle: Text('$selectClass Quiz', style: TextStyle(fontSize: 12)), //
                                        trailing: const Icon(Icons.visibility, color: Colors.purple),
                                      ),
                                    ),
                                  ),
                                );
