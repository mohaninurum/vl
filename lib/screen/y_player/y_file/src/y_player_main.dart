...
import 'package:visual_learning/screen/y_player/y_file/src/quality_selection_sheet.dart';
import 'package:visual_learning/screen/y_player/y_file/src/speed_slider_sheet.dart';
...
import 'package:visual_learning/screen/y_player/y_file/src/utils/y_player_default_enter_screen.dart';
import 'package:visual_learning/screen/y_player/y_file/src/utils/y_player_default_exit_full_screen.dart';
import 'package:visual_learning/screen/y_player/y_file/src/y_player_controller.dart';
...
class YPlayer extends StatefulWidget {
  ...
  const YPlayer({super.key, required this.youtubeUrl, this.aspectRatio, this.autoPlay = true, this.placeholder, this.loadingWidget, this.errorWidget, this.onStateChanged, this.onProgressChanged, this.onControllerReady, this.color, this.onEnterFullScreen, this.onExitFullScreen, this.seekBarMargin, this.fullscreenSeekBarMargin, this.bottomButtonBarMargin, this.fullscreenBottomButtonBarMargin, this.chooseBestQuality = true, this.forceOriginalAudio = false});
...
class YPlayerState extends State<YPlayer> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _controller = YPlayerController(onStateChanged: widget.onStateChanged, onProgressChanged: widget.onProgressChanged);
      ...
      builder: (context, constraints) {
        ...
        return ValueListenableBuilder<YPlayerStatus>(
          ...
          builder: (context, status, _) {
            return Container(width: playerWidth, height: playerHeight, color: Colors.transparent, child: _buildPlayerContent(playerWidth, playerHeight, status));
          },
        );
      },
    );
  ...
  void _showQualitySelector(BuildContext context) {
    ...
    if (qualityOptions.isEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('No quality options available')));
      return;
    }
  ...
  Widget _buildPlayerContent(double width, double height, YPlayerStatus status) {
    if (_isControllerReady && _controller.isInitialized) {
      ...
      return MaterialVideoControlsTheme(
        normal: MaterialVideoControlsThemeData(seekBarBufferColor: Colors.grey, seekOnDoubleTap: true, seekBarPositionColor: widget.color ?? const Color(0xFFFF0000), seekBarThumbColor: widget.color ?? const Color(0xFFFF0000), seekBarMargin: widget.seekBarMargin ?? EdgeInsets.zero, bottomButtonBarMargin: widget.bottomButtonBarMargin ?? const EdgeInsets.only(left: 16, right: 8), brightnessGesture: true, volumeGesture: true, bottomButtonBar: [const MaterialPositionIndicator(), const Spacer(), IconButton(icon: const Icon(Icons.hd_outlined, color: Colors.white), onPressed: () => _showQualitySelector(context)), IconButton(icon: const Icon(Icons.speed, color: Colors.white), onPressed: () => _showSpeedSlider(context)), const MaterialFullscreenButton()]),
        fullscreen: MaterialVideoControlsThemeData(volumeGesture: true, brightnessGesture: true, seekOnDoubleTap: true, seekBarMargin: widget.fullscreenSeekBarMargin ?? EdgeInsets.zero, bottomButtonBarMargin: widget.fullscreenBottomButtonBarMargin ?? const EdgeInsets.only(left: 16, right: 8), seekBarBufferColor: Colors.grey, seekBarPositionColor: widget.color ?? const Color(0xFFFF0000), seekBarThumbColor: widget.color ?? const Color(0xFFFF0000), bottomButtonBar: [const MaterialPositionIndicator(), const Spacer(), IconButton(icon: const Icon(Icons.hd_outlined, color: Colors.white), onPressed: () => _showQualitySelector(context)), IconButton(icon: const Icon(Icons.speed, color: Colors.white), onPressed: () => _showSpeedSlider(context)), const MaterialFullscreenButton()]),
        child: Video(
